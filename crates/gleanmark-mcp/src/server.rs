...
use gleanmark_core::models::{BookmarkInput, SearchQuery};
...
impl GleanMarkMcp {
    pub fn new(gm: Arc<GleanMark>) -> Self {
...
impl GleanMarkMcp {
    ...
    async fn search(&self, Parameters(params): Parameters<SearchParams>) -> String {
        ...
        match self.gm.search(query).await {
            Ok(results) => {
                if results.is_empty() {
    ...
    async fn list(&self, Parameters(params): Parameters<ListParams>) -> String {
        ...
        match self.gm.list(limit, None).await {
            Ok(bookmarks) => {
    ...
    async fn add(&self, Parameters(params): Parameters<AddBookmarkParams>) -> String {
        ...
        match self.gm.save_bookmark(input).await {
    ...
    async fn delete(&self, Parameters(params): Parameters<DeleteParams>) -> String {
        match self.gm.delete(&params.id).await {
